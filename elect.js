...
// var Aerocool = (function(){
//   var _this;
//   env.log('666','666','666','666');
// })
...
app.on('ready', function () {
    ...
    // global.MainWindow = new windows.AerocoolClass();
...
var Smart = (function () {
    ...
    function Smart(opts) {
        ...
        global.MainWindow.on('finish-load', function (isCrashed) {   
            ...
            if(global.AppProtocol !== undefined){
                _this.initDevice();
                var labs = [];
                labs.push('Open Configuration');labs.push('Exit');
                _this.installTray(labs);
            }else{
                global.AppProtocol = new protoInterface.SmartProtocolClass();
                _this.initDevice();
            }
            global.AppProtocol.on(evtVar.ProtocolMessage, function(obj){
                env.log(env.level.INFO,'main','AppProtocol',JSON.stringify(obj))
    ...
    Smart.prototype.registerListeners = function () {
        ...
        ipcMain.on("SystemIcon_Loading", function (event, labs){ 
            _this.installTray(labs);        
        });
    };
    ...
    Smart.prototype.installTray = function (labs) { 
        ...
        global.TrayIcon = new Tray(path.join(__dirname,'image/logo.png')); 
        ...
        //global.TrayIcon.setTitle("TestApp");
        global.TrayIcon.setToolTip("Aerocool");
        ...
        if(!_this._initDev){
            ...
            global.AppProtocol.RunFunction(Obj, function (error,data){
                try
                {
                    ...
                    env.log(`AppProtocol.InitDevice Error : ${ex}`);
                }
            });
        }
    };
