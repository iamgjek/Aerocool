...
var SmartProtocol = (function (_super) {
    ...
    SmartProtocol.prototype.RunFunction = function (Obj, callback) {
        try{  
        	...
        	if(!_this.CheckParam(Obj)){
        		callback('Error','SmartProtocol.RunFunction');
        		return;
        	}
    ...
    SmartProtocol.prototype.CheckParam = function (Obj) {
	    ...
	    // 	return false;
        if(!Obj.hasOwnProperty('Type')||!Obj.hasOwnProperty('SN')||!Obj.hasOwnProperty('Func')||!Obj.hasOwnProperty('Param'))
            return false
    ...
    SmartProtocol.prototype.InitDevice = function (callback) { 
        ...
        try{       
            if(_this.SupportDevice == undefined){
                _this.AppDB.getDevice(function(data){
                    _this.SupportDevice = data;
                    Promise.all([_this.DeviceApi.InitDevice(_this.SupportDevice)]).then(function() {
                        callback();
                    },function(){
        ...
        try{
            for(var device of _this.SupportDevice){
                if( Number(device.VID)== Number(baseInfo.VID) && Number(device.PID)== Number(baseInfo.PID))
                {
                    ...
                    if(USBData.PlugType == 1)
                        device.status = "1";
                    else if(USBData.PlugType == 0)
                        device.status = "0";
                    _this.AppDB.updateDevice(device._id, device).then(function(doc){
                        var Obj={
    ...
    SmartProtocol.prototype.DeviceApi = undefined;
    //支援机种
